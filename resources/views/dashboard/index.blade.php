...
@php($senderReady = app(\App\Services\Email\MerchantSenderService::class)->configured() && app(\App\Services\Email\MerchantSenderService::class)->ready($shop))
@if($shop->settings?->auto_email_enabled && $shop->settings?->onboarded_at && !config('chargeguard.test_mode') && !$shop->settings?->test_mode && (!config('senders.required') || $senderReady))
...
@if(config('senders.required') && !$senderReady)<s-paragraph>Verify your sending domain before activating customer emails.</s-paragraph><s-link href="/settings/email-sender">Configure email sender</s-link>@endif
