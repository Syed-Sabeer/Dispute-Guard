...
class SendTestAutomationEmail extends QueuedJob
{
    ...
    public function handle(EmailComposer $composer): void
    {
        ...
        $shop = $log->shop;
        if (!$shop->active()) { $log->update(['status'=>'CANCELLED']); return; }
        $template = EmailTemplate::forShop($shop)->find($log->email_template_id);
        if (!$template) { $log->update(['status'=>'CANCELLED']); return; }
        ...
        if (!EmailLog::whereKey($log->id)->where('status','QUEUED')->update(['status'=>'SENDING'])) { return; }
        try {
            $shop->refresh();
            if (!$shop->active()) { $log->update(['status'=>'CANCELLED']); return; }
        ...
        } catch (\Throwable) { $log->update(['status'=>'FAILED','error_message'=>'Test email failed or delivery outcome is uncertain.']); }
    }
}
