...
class ShopifyAppService
{
    ...
    {
        return new Client(['timeout' => 12, 'connect_timeout' => 4]);
    }
    ...
    public function token(Shop $shop): array
    {
        return Cache::lock('shopify-token-'.$shop->id, 25)->block(3, function () use ($shop) {
            $shop->refresh();
            if (! $shop->active() || ! $shop->access_token) {
                throw new ShopifyApiException(false, 'Reconnect the app from Shopify Admin.');
            }
            try {
            ...
            if (! $result->ok) {
                throw new ShopifyApiException($result->response->status >= 500);
            }
