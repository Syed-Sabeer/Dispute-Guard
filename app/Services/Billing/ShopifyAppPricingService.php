...
class ShopifyAppPricingService implements BillingServiceInterface
{
    ...
    {
        if (app()->environment(['local','testing']) && !config('chargeguard.billing_enforced')) { return true; }
        if (!$shop->active()) { return false; }
        $partner = config('shopify.partner_id'); $token = config('shopify.partner_token');
        $app = config('shopify.app_id'); $version = config('shopify.partner_api_version');
        if (!$partner || !$token || !$app || !$shop->shopify_shop_id || !ctype_digit((string) $partner) || !preg_match('/^20\d{2}-\d{2}$/D', $version)) { return false; }
        try {
            ...
            if (!$response->successful() || $response->json('errors') || !is_array($response->json('data'))) { throw new \RuntimeException(); }
            $sub = $response->json('data.activeSubscription');
            $items = collect($sub['items'] ?? [])->filter(fn ($i) => data_get($i, 'price.active') === true);
            $matched = $items->first(fn ($i) => in_array($i['handle'], array_values(config('chargeguard.billing')), true));
            $active = $sub !== null && $matched !== null;
            $shop->update(['billing_status'=>$active ? 'ACTIVE' : 'INACTIVE','plan_handle'=>$matched['handle'] ?? null,'billing_checked_at'=>now()]);
            return $active;
        } catch (\Throwable) {
    ...
    public function manageUrl(Shop $shop): ?string
    {
        ...
        if (!$handle || !preg_match('/^[a-z0-9-]+$/D', $handle)) { return null; }
