...
{
    public function resolve(array $order): State { return $this->inspect($order)['state']; }
    public function inspect(array $order): array
    {
        $unknown = ['state'=>State::UNKNOWN,'raw'=>'INCOMPLETE','tracking'=>[]];
        if (!array_key_exists('fulfillments', $order) || !is_array($order['fulfillments'])) { return $unknown; }
        ...
        if (in_array($overall, ['PARTIALLY_FULFILLED','UNFULFILLED','ON_HOLD','SCHEDULED'], true) || count($fulfillments) >= 250) { return $unknown; }
        $states = []; $raw = []; $tracking = [];
        foreach ($fulfillments as $f) {
            if (($f['status'] ?? '') !== 'SUCCESS') { return $unknown; }
            ...
            if (!$info || data_get($f, 'events.pageInfo.hasNextPage', false)) { return $unknown; }
            ...
            $states[] = $state; $raw[] = $status; $tracking = array_merge($tracking, $info);
        }
        // A single message cannot accurately describe mixed shipments; never infer complete delivery.
        if (count(array_unique(array_map(fn ($s) => $s->value, $states))) !== 1) { return ['state'=>State::UNKNOWN,'raw'=>implode(', ', $raw),'tracking'=>$tracking]; }
