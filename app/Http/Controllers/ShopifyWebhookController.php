...
class ShopifyWebhookController extends Controller
{
    public function __invoke(Request $request, string $category, string $action, ShopifyAppService $app)
    {
        ...
        abort_unless(ShopifyRequestVerifier::domain((string) $request->header('X-Shopify-Shop-Domain')) === $domain, 400);
        ...
        DB::transaction(function () use ($shop, $domain, $id, $topic, $payload, $request) {
            ...
            if (! $shop) {
                $event->update(['status' => 'IGNORED', 'payload' => null, 'error_message' => 'Shop not installed.', 'processed_at' => now()]);
            ...
            if (str_starts_with($topic, 'disputes/')) {
                ...
                $job = new ProcessPrivacyWebhook($event->id);
            }
