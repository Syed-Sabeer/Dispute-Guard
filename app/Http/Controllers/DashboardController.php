...
class DashboardController extends MerchantController
{
    ...
    {
        $shop = $this->shop(); $q = $shop->disputes()->where('source','shopify');
        $metrics = [
        ...
        foreach (['IN_TRANSIT','OUT_FOR_DELIVERY','DELIVERED'] as $state) { $metrics[ucwords(strtolower(str_replace('_',' ',$state)))] = (clone $q)->where('shipping_state',$state)->count(); }
