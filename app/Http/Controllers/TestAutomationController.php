...
class TestAutomationController extends MerchantController
{
    ...
    {
        return $this->page('test-automation.index');
    }
    ...
    public function send(SendTestAutomationRequest $request)
    {
        ...
        try {
            ...
            throw ValidationException::withMessages(['sender_email' => 'Verify your sender email before sending a test automation email.']);
        }
        ...
        DB::transaction(function () use ($shop, $data, $template, $senderKey) {
            ...
            SendTestAutomationEmail::dispatch($log->id, Crypt::encryptString(json_encode($data, JSON_THROW_ON_ERROR)), $senderKey)->onConnection('database');
        });
