...
class EmailTemplateController extends MerchantController
{
    ...
    {
        return $this->page('templates.index', ['groups' => $this->shop()->emailTemplates()->get()->groupBy('dispute_reason')]);
    }
    ...
    public function edit(EmailTemplate $template)
    {
        ...
        return $this->page('templates.edit', ['template' => $template]);
    }
    ...
    public function update(UpdateEmailTemplateRequest $request, EmailTemplate $template, TemplateRenderer $renderer)
    {
        ...
        $data['body'] = $renderer->sanitize($data['body']);
        $template->update($data + ['is_default_modified' => true]);
    ...
    {
        $this->owned($template);
        abort_unless($request->boolean('confirmed'), 422, 'Confirm restoration first.');
