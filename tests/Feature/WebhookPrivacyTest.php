...
use App\Models\{WebhookEvent,Dispute,EmailLog,PrivacyRequest};
use App\Jobs\{ProcessDisputeCreated,ProcessPrivacyWebhook};
...
class WebhookPrivacyTest extends TestCase
{
    ...
    {
        Queue::fake(); $shop = $this->shop();
        $this->webhook($shop)->assertOk(); $this->webhook($shop)->assertOk();
        $this->assertDatabaseCount('webhook_events',1); Queue::assertPushed(ProcessDisputeCreated::class,1);
    ...
    {
        Queue::fake(); $shop = $this->shop(); $shop->delete();
        $this->webhook($shop)->assertOk(); Queue::assertNothingPushed();
        $this->assertSame('IGNORED',WebhookEvent::sole()->status);
    }
    public function test_webhook_job_is_durable_in_database_queue(): void
    {
        $shop = $this->shop(); $this->webhook($shop)->assertOk();
    ...
    {
        $shop = $this->shop(); $this->webhook($shop,'app/uninstalled')->assertOk();
        $this->assertFalse($shop->fresh()->active()); $this->assertNull($shop->fresh()->access_token);
    ...
    public function test_privacy_export_and_customer_redaction(): void
    {
        ...
        $this->assertSame('READY',PrivacyRequest::sole()->status); $this->assertCount(1,PrivacyRequest::sole()->export['disputes']);
        ...
        $this->assertNull($d->fresh()->tracking_number); $this->assertNotNull($d->fresh()->redacted_at);
        $this->assertNull($log->fresh()->rendered_body); $this->assertNull(PrivacyRequest::sole()->export);
    }
    public function test_shop_redaction_removes_tenant_data(): void
    {
        Queue::fake(); $shop = $this->shop(); $this->webhook($shop,'app/uninstalled','uninstall')->assertOk();
        ...
        $this->assertDatabaseMissing('shops',['id'=>$shop->id]); $this->assertDatabaseCount('email_templates',0);
    }
}
