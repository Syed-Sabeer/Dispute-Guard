...
class MerchantTest extends TestCase
{
    ...
    public function test_signed_authenticated_pages_render(): void
    {
        $shop = $this->shop();
        $this->merchant($shop);
        $verified = app(ShopifyAppService::class)->sdk()->verifyAppHomeReq(['method' => 'GET', 'url' => 'http://localhost/dashboard', 'body' => '', 'headers' => ['authorization' => 'Bearer '.$this->token($shop)]], '/auth/patch-id-token');
        $this->assertTrue($verified->ok, $verified->log->code.' '.$verified->log->detail);
        foreach (['/dashboard', '/onboarding', '/disputes', '/templates', '/test-automation', '/email-logs', '/settings', '/billing', '/privacy-requests', '/templates/'.$shop->emailTemplates()->first()->id.'/edit'] as $url) {
    ...
    public function test_tenant_isolation(): void
    {
        $a = $this->shop();
        $b = $this->shop();
        $dispute = Dispute::factory()->create(['shop_id' => $b->id]);
        $template = $b->emailTemplates()->first();
    ...
    public function test_test_email_uses_real_rendering_and_mailable(): void
    {
        ...
        $this->assertSame('test', EmailLog::sole()->type);
        ...
        app()->call([$job, 'handle']);
        Mail::assertSent(DisputeCustomerMail::class, fn ($mail) => $mail->hasTo('developer@example.com') && str_starts_with($mail->mailSubject, '[TEST]'));
    }
