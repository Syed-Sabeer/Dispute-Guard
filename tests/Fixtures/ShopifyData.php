...
trait ShopifyData
{
    ...
    protected function token(Shop $shop, array $claims = []): string
    {
        return JWT::encode($claims + ['iss' => 'https://'.$shop->shop_domain.'/admin', 'dest' => 'https://'.$shop->shop_domain, 'aud' => config('shopify.api_key'), 'sub' => '1', 'exp' => time() + 60, 'nbf' => time() - 1, 'iat' => time(), 'jti' => 'test-jti', 'sid' => 'test-sid'], config('shopify.api_secret'), 'HS256');
    }

    protected function merchant(Shop $shop): static
    {
    ...
    {
        $this->mock(ShopifyOrderService::class, fn ($m) => $m->shouldReceive('fetch')->withArgs(fn ($s, $id) => $id === 'gid://shopify/Order/456')->andReturn($order));
        $this->mock(ShopifyDisputeService::class, fn ($m) => $m->shouldReceive('fetch')->andReturn($dispute ?? $this->disputeData()));
    }

    protected function webhook(Shop $shop, string $topic = 'disputes/create', string $id = 'webhook-1', array $payload = ['id' => 789], bool $valid = true)
    {
